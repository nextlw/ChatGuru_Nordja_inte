...
impl SmartFolderFinder {
    ...
    pub async fn find_folder_for_client(&mut self, client_name: &str) -> AppResult<Option<FolderSearchResult>> {
    ...
    async fn search_folders_via_api(&self, normalized_client: &str) -> AppResult<Option<FolderSearchResult>> {
        tracing::info!("📡 Buscando folders via API do ClickUp...");

        // GET /team/{team_id}/space/{space_id}/folder
        // Como não sabemos o space_id, vamos buscar em todos os spaces
        // Alternativa: GET /team/{team_id}/folder (se disponível)

        let url = format!("https://api.clickup.com/api/v2/team/{}/space", self.team_id);
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| AppError::InternalError(format!("Falha ao buscar spaces: {}", e)))?;
        ...
        let spaces: serde_json::Value = response.json().await
            .map_err(|e| AppError::InternalError(format!("Falha ao parsear spaces: {}", e)))?;

        let spaces_array = spaces["spaces"].as_array()
            .ok_or_else(|| AppError::InternalError("Campo 'spaces' não é array".to_string()))?;

        // Para cada space, buscar folders
        let mut all_folders = Vec::new();
        for space in spaces_array {
            let space_id = space["id"].as_str()
                .ok_or_else(|| AppError::InternalError("Space sem ID".to_string()))?;
    ...
    async fn fetch_folders_from_space(&self, space_id: &str) -> AppResult<Vec<ClickUpFolder>> {
        let url = format!(
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| AppError::InternalError(format!("Falha ao buscar folders: {}", e)))?;
        ...
        let folders_response: ClickUpFoldersResponse = response.json().await
            .map_err(|e| AppError::InternalError(format!("Falha ao parsear folders: {}", e)))?;
    ...
    ) -> AppResult<Option<FolderSearchResult>> {
    ...
    async fn search_historical_tasks(&self, normalized_client: &str) -> AppResult<Option<FolderSearchResult>> {
        tracing::info!("🕐 Buscando tarefas históricas com 'Cliente Solicitante' = '{}'", normalized_client);

        // GET /team/{team_id}/task
        // Filtrar por custom_field "Cliente Solicitante"
        let url = format!(
        ...
        let response = self.client
            ...
            .query(&[
                ("archived", "false"),
                ("subtasks", "false"),
                ("include_closed", "true"), // Incluir tarefas fechadas (histórico)
            ])
            .send()
            .await
    ...
    async fn find_or_create_current_month_list(&self, folder_id: &str) -> AppResult<(String, String)> {
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| AppError::InternalError(format!("Falha ao buscar folder: {}", e)))?;
    ...
    async fn create_list(&self, folder_id: &str, list_name: &str) -> AppResult<(String, String)> {
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| AppError::InternalError(format!("Falha ao criar lista: {}", e)))?;
        ...
        let list: serde_json::Value = response.json().await
            .map_err(|e| AppError::InternalError(format!("Falha ao parsear lista criada: {}", e)))?;
